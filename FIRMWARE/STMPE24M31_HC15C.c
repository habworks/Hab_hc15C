...
const uint8_t STMPE24M31_SETUP_REGISTER_ARRAY[][2] = 
  {
  // BASIC TOUCH SETUP
  {0x03, 0x08}, // SYSCON-1: SLEEP ENABLE MODE - ???
  ...
  {0x77, 0x55}, // CAL_MODE: CALIBRATION
  {0x76, 0x06}, // CAL_INT: CALIBRATION INTERVAL
...
volatile uint32_t ACAP_TouchKey, BCAP_TouchKey;
volatile uint8_t KeyPressed_A = FALSE, KeyPressed_B = FALSE;
 ...
 *************************************************************************/
 uint8_t STMPE24M31_write(uint8_t *RegisterWriteArray, uint8_t RegisterSize, uint8_t STMPE24M31_SlaveAddress)
 {
 ...
 * using sizeof(Array) operator.  Last parameter is the slave address of the EEPROM in 7 bit form right shifted. 
 * NOTE: Uses CMSIS I2C transfer functions.  Requires the I2C be previously initialize
 ...
 *************************************************************************/
 uint8_t STMPE24M31_read(uint8_t *RegisterToReadArray, uint8_t RegisterBytesToRead, uint8_t RegisterAddress, uint8_t STMPE24M31_SlaveAddress)
 {
  // I2CM TYPDEF
 ...
 *************************************************************************/
 uint8_t STMPE24M31_init(uint8_t STMPE24M31_Device)
 {
 
 // STEP 1
 uint8_t DeviceAddress, ArraySize;
 uint8_t RegisterAndValue[2];
 uint8_t KeyFilterRegister[4];
 uint8_t INT_STA_Register[2];
...
/*************************************************************************
 * Function Name: ctl_STMPE24M31_EINT
 * Parameters:    CTL_ISR_FN_t
 * Return:        void
 *
 * Description: Sets up the External IRQs(2 and 1) for STMPE24M31 device A and B
 ...
 *************************************************************************/
 void ctl_STMPE24M31_EINT(CTL_ISR_FN_t buttonFn)
 {
  
  // TYPEDEF
  ISR_EINT1_Var = buttonFn;
  PINSEL_CFG_Type PinCfgStruct;
  ...
  // STEP 4
  // EXTI 1 - B DEVICE
  ctl_set_priority(EINT1_IRQn, 2);
  ctl_unmask_isr(EINT1_IRQn);
  // EXTI 2 - A DEVICE
  ctl_set_priority(EINT2_IRQn, 3);
  ctl_unmask_isr(EINT2_IRQn);
   
 } //END OF FUNCTION ctl_STMPE24M31_EINT
 
 
 
 
/*************************************************************************
 * Function Name: void EINT1_IRQHandler
 * Parameters:    void
 * Return:        void
 *
 * Description: IRQ Handler for External IRQ (STMPE24M31 Device B)
 * STEP 1: 
 * STEP 2: 
 * STEP 3: 
 * STEP 4: 
...
/*************************************************************************
 * Function Name: void EINT2_IRQHandler
 * Parameters:    void
 * Return:        void
 *
 * Description: IRQ Handler for External IRQ (STMPE24M31 Device A)
 * STEP 1: 
 * STEP 2: 
 * STEP 3: 
 * STEP 4: 
 ...
 *************************************************************************/
 ...
 if (LED_ON)
   {
   LED_ON = FALSE;
   switchLED(LED_OFF, LED_RAD);
   }
 else
   {
   LED_ON = TRUE;
   switchLED(LED_ON, LED_RAD);
   }
 ...
 switch(INT_STA_ReadRegister[0])
   ...
   // LOAD EITHER THE KEY FILTER REGISTERS
   KeyFilterRead = 0x00;
   ...
   // LOAD EITHER ACAP or BCAP TOUCH BASED ON KEY PRESSED
   if ((DeviceAddress == SLAVE_ADDRESS_STMPE24M31_ACAP) && (KeyFilterRead != 0))
     {
     ACAP_TouchKey = KeyFilterRead;
     KeyPressed_A = TRUE;
     }   
   if ((DeviceAddress == SLAVE_ADDRESS_STMPE24M31_BCAP) && (KeyFilterRead != 0))
     {
     BCAP_TouchKey = KeyFilterRead;
     KeyPressed_B = TRUE;
     }
   break;
 ...
 * STEP 4: return
 *************************************************************************/
 uint8_t STMPE24M31_reset(uint8_t STMPE24M31_Device)
 {
 
 // STEP 1
 uint8_t SYSCON3_WriteRegister[2];
 ...
 // STEP 3
 SYSCON3_WriteRegister[VALUE_BYTE] = SYSCON3_SOFT_RESET;
 SYSCON3_WriteRegister[REGISTER_BYTE] = STMPE24M31_SYSCON3_REG;
 STMPE24M31_write(SYSCON3_WriteRegister, sizeof(SYSCON3_WriteRegister), DeviceAddress);
