...
#include <cross_studio_io.h> // Added this for CrossWorks Debug Terminal
#include <stdarg.h>
#include "monitor.h"
...
)
{
	...
	debug_printf("%08lX ", addr);		/* address */
...
)
{
	...
	rv = i = 0;
	for (;;) {
 		c = debug_getch();				/* Get a char from the incoming stream */
		if (!c) return 0;			/* End of stream? */
				
		/*if (c == '\b' && i) 
    {		// Back space? 
			i--;
			//take this out for now if (_LINE_ECHO) debug_putchar(c);
			debug_putchar(' '); // mthomas
			debug_putchar(c);   // mthomas
			continue;
		}*/
		if ((c >= ' ') && (i < len - 1)) 
      {	/* Visible chars */
			buff[i++] = c;
			if (_LINE_ECHO) debug_putchar(c);
		  }
    if (c == 0x0A) break; /* End of line? '\r'= 13 and '\n' = 10*/    
	}
	buff[i] = 0;	/* Terminate with zero */
	if (_LINE_ECHO) debug_putchar('\n');
	return 1;
}
